...
def universal_node(num_mini_pool, neurons_per_pool, dropout_prob=0.1, num_refine=1, auto_regressive=False, device='cuda'):
    total_neurons = num_mini_pool * neurons_per_pool
    parameters = {
        'pre_synaptic': torch.nn.Parameter(torch.zeros((num_mini_pool, neurons_per_pool, neurons_per_pool), device=device).normal_(std=0.02)),
        'post_synaptic': torch.nn.Parameter(torch.zeros((num_mini_pool, neurons_per_pool, neurons_per_pool), device=device).normal_(std=0.02)),
        'bind_synapse': torch.nn.Parameter(torch.zeros((total_neurons, neurons_per_pool), device=device).normal_(std=0.02)),
    }
    ...
    def forward(input_features, train_mode=True):
        ...
        return normalized_features.view(batch, num_tokens, neurons_per_pool)
