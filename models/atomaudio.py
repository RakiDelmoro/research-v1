...
from utils.data_utils import audio_dataloader
from utils.model_utils import save_checkpoint
from models.configs import AudioConfigs
...
def model_and_parameters(device):
    ...
    parameters['conv2d_weights'] = torch.nn.Parameter(torch.zeros(configs.neurons_per_mini_pool, 1, configs.audio_patch_size, configs.audio_patch_size, device=device).normal_(std=0.02), requires_grad=False)
...
def audio_model_runner(max_epochs, batch_size, device='cuda', learning_rate=1e-3):
    ...
    for epoch in range(1, max_epochs+1):
        train_loader = audio_dataloader(train_x, train_y, batch_size=batch_size, shuffle=True)
        validation_loader = audio_dataloader(test_x, test_y, batch_size=batch_size, shuffle=False)
